...
# Get phonetic similarity score between two sequences

>>> similarity('θin', 'tenwis') # doctest: +SKIP
0.6719...
...
def align(str1, str2, epsilon=0):
    ...
    for i in range(1, m + 1):
        for j in range(1, n + 1):
            ...
            edit4 = S[i - 2, j - 1] + sigma_exp(str2[j - 1], str1[i - 2:i]) if i > 1 else -inf
            edit5 = S[i - 1, j - 2] + sigma_exp(str1[i - 1], str2[j - 2:j]) if j > 1 else -inf
...
def similarity(str1, str2):
    ...
    Returns a score in [0, 1] where 1.0 means the strings are identical
    and 0.0 means maximally dissimilar. The raw alignment score is
    normalized by the maximum possible score, which is the self-alignment
    score of the longer (higher-scoring) word.

    :param str str1: First phonetic string
    :param str str2: Second phonetic string
    ...
    score = _align_score(str1, str2)
    denom = max(_align_score(str1, str1), _align_score(str2, str2))
...
def _align_score(str1, str2):
    """
    Returns the raw optimal alignment score between two strings.
    Same DP as align() but just returns the max cell value.
    """
    m = len(str1)
    n = len(str2)
    S = np.zeros((m + 1, n + 1), dtype=float)
...
def delta(p, q):
    ...
    features = R(p, q)
    return np.dot(
        [diff(p, q, f) for f in features],
        [salience[f] for f in features],
    )
...
def V(p):
    """
    Return vowel weight if P is a vowel.
...
def demo():
    """
    Computes phonetic similarity scores for cognate pairs from Kondrak (2002).
    Scores closer to 1.0 indicate more phonetically similar word pairs.
    """
    data = [pair.split(",") for pair in cognate_data.split("\n")]
    print(f"{'Word 1':<25} {'Word 2':<25} {'Similarity':>10}")
    print("-" * 62)
    for pair in data:
        str1, str2 = pair[0], pair[1]
        score = similarity(str1, str2)
        print(f"{str1:<25} {str2:<25} {score:>10.4f}")
